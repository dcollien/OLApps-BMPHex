...
BMPHexEditor = (function() {
  ...
  BMPHexEditor.ACHIEVEMENT_INIT_ORDER = ['Instructions', 'Show Pixel Array', 'Show DIB Header', 'Show BMP Header', 'Edited Padding Bytes'];
  ...
  function BMPHexEditor(width, height, unlockedAchievements, pixels) {
    ...
    if (this.unlockedAchievements) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (unlockedAchievements[name]) {
          this.unlockAchievement(name, true);
        }
      }
    } else {
    ...
    this.resetImage(pixels, width, height);
    ...
    if (window.app) {
      window.app.ready(function() {
        return _this.unlockAchievement('Instructions', false, false);
      });
    } else {
      this.unlockAchievement('Instructions', false, false);
    }
  }

  BMPHexEditor.prototype.unlockAchievement = function(achievement, isSilent, timeout) {
    ...
    if (this.unlockedAchievements[achievement] === true) {
      return;
    }
    if (!window.app) {
    ...
    if (!isSilent && !window.app) {
      ...
      return window.app.sendMessage('notify', {
        'title': achievementData.title,
    ...
    reader.onload = function(readerEvent) {
      ...
      image.onload = function(imageEvent) {
        if (image.width <= BMPHexEditor.MAX_DIMENSION && image.height <= BMPHexEditor.MAX_DIMENSION) {
          ...
          return alert('Image is too large for this activity (' + BMPHexEditor.MAX_DIMENSION + 'px x ' + BMPHexEditor.MAX_DIMENSION + 'px maximum)');
        }
      };
  ...
  BMPHexEditor.prototype.updatePixels = function() {
    var $byteDisplay, bytes, i, _i, _j, _ref, _ref1;
    bytes = this.getBytes();
    this.pixels = [];
    for (i = _i = 0, _ref = this.width * this.height * 4; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
      this.pixels.push(this.getPixelFromBMPBytes(i, bytes));
    }
    for (i = _j = 0, _ref1 = bytes.length; 0 <= _ref1 ? _j < _ref1 : _j > _ref1; i = 0 <= _ref1 ? ++_j : --_j) {
  ...
  BMPHexEditor.prototype.redraw = function() {
    var b, bytes, g, heightPixelSize, i, pixelSize, r, widthPixelSize, x, y, _i, _ref, _results;
    bytes = this.getBytes();
    this.base64Data = this.encodeDataURI(this.hexArrayToDec(bytes));
    $('#download-link').attr('href', 'data:image/bmp;base64,' + this.base64Data);
    $('#preview-img').attr('src', 'data:image/bmp;base64,' + this.base64Data);
      ...
      _results.push((function() {
        ...
        for (y = _j = 0, _ref1 = this.height; 0 <= _ref1 ? _j < _ref1 : _j > _ref1; y = 0 <= _ref1 ? ++_j : --_j) {
          ...
          if (this.isSelected && this.selectedX === x && this.selectedY === y) {
            ...
            _results1.push(this.ctx.strokeRect(x * pixelSize + 1, y * pixelSize + 1, pixelSize - 2, pixelSize - 2));
          } else {
  ...
  BMPHexEditor.prototype.hexArrayToDec = function(bytes) {
    ...
    for (_i = 0, _len = bytes.length; _i < _len; _i++) {
      byte = bytes[_i];
      _results.push(parseInt(byte, BMPHexEditor.HEX));
    }
    return _results;
  };
  ...
  BMPHexEditor.prototype.rebuildFileDisplay = function() {
    var $byteField, $fileByteDisplay, bytes, className, hex, i, paddedHex, self, tagName, _i, _ref,
      _this = this;
  ...
  BMPHexEditor.prototype.headerByteChanged = function() {
    if (!this.headerBytesChanged) {
      this.headerBytesChanged = 0;
    }
    this.headerBytesChanged += 1;
    if (this.headerBytesChanged > 2) {
      this.unlockAchievement('Show DIB Header');
    }
    return this.resetImage();
  };

  BMPHexEditor.prototype.pixelByteChanged = function() {
    var a, b, colorString, colorsSeen, g, i, offset, r, _i, _ref, _results;
    ...
    if (this.pixelsChanged > 8) {
      ...
      for (i = _i = 0, _ref = this.width * this.height; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        ...
        if (colorsSeen.length > 3) {
          this.unlockAchievement('Show Pixel Array');
        }
  ...
  BMPHexEditor.prototype.getBytes = function() {
    var $byteDisplay, byteValue, bytes, i, _i, _ref;
    bytes = [];
    for (i = _i = 0, _ref = this.getFileLength(); 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
      $byteDisplay = $('#byte' + i);
  ...
  BMPHexEditor.prototype.rebuild = function() {
    ...
    this.rebuildFileDisplay();
    return this.resize();
  };

  BMPHexEditor.prototype.rebuildPixelArrayInput = function() {
    var $input, $pixelArray, byteValue, col, i, paddingPerRow, pixelNum, self, value, _i, _ref,
      _this = this;
    $pixelArray = $('#pixelArrayData');
    $pixelArray.empty();
    pixelNum = 0;
    for (i = _i = 0, _ref = this.getPixelArrayLength(); 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
    ...
    return paddingPerRow = this.getRowLength() - this.width * BMPHexEditor.BYTES_PER_PIXEL;
  };

  BMPHexEditor.prototype.getDimensions = function() {
    var height, width;
  ...
  BMPHexEditor.prototype.resetImage = function(pixels, width, height) {
    var i, _i, _ref;
    if (width != null) {
      this.set4ByteVal(BMPHexEditor.WIDTH_BYTES, width);
    } else {
      width = this.get4ByteVal(BMPHexEditor.WIDTH_BYTES);
    }
    if (height != null) {
      this.set4ByteVal(BMPHexEditor.HEIGHT_BYTES, height);
    } else {
      height = this.get4ByteVal(BMPHexEditor.HEIGHT_BYTES);
    }
